...
public class RmWorker extends ComponentDefinition {
    private static final Logger logger = LoggerFactory.getLogger(RmWorker.class);
    ...
    Handler<AllocateResources> handleAllocateResources = new Handler<AllocateResources>() {
        ...
        public void handle(AllocateResources event) {
        	Task t = new Task(event.getId(), event.getNumCpus(), event.getMemoryInMbs(), event.getTimeToHoldResource());
        	waiting.add(t);
        	pop();
        }
    };
    ...
    Handler<TaskDone> handleTaskDone = new Handler<TaskDone>() {
		...
		public void handle(TaskDone event) {
			...
			logger.info("Done {}, QueueTime={}, TotalTime={}",
				new Object[] { t.getId(),
					t.getQueueTime(),
					(float)(t.getTimeToHoldResource()) / t.getTotalTime()
				}
			);
